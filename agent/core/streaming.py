...
class StreamingReporter:
    ...
    def start_session(self, task_description: str):
        ...
        self.console.print(f"\n🤖 [bold blue]Starting Development Task[/bold blue]")
        self.console.print(f"📝 {task_description}\n")
        self.start_time = time.time()
        
    def on_planning_start(self):
        """Called when planning phase begins."""
        with self.console.status("[bold green]🧠 Planning task..."):
            time.sleep(0.1)  # Brief pause for visual effect
            
    def on_plan_created(self, plan: list[str]):
        """Called when plan is ready."""
        self.total_steps = len(plan)
        self.console.print("📋 [bold green]Plan created:[/bold green]")
    ...
    def on_execution_start(self):
        ...
        self.console.print("🚀 [bold green]Starting execution...[/bold green]\n")
    ...
    def on_tool_call(self, tool: str, args: Dict[str, Any]):
        ...
        if self.verbose:
            ...
            self.console.print(f"  🔧 {tool}({args_str})")
    ...
    def on_thinking(self, thought: str):
        ...
        if self.verbose:
            self.console.print(f"💭 [italic dim]{thought}[/italic dim]")
    ...
    def on_session_complete(self, success: bool, summary: str = ""):
        ...
        if success:
            self.console.print(f"\n🎉 [bold green]Task completed successfully![/bold green] ({time_str})")
        else:
            self.console.print(f"\n💥 [bold red]Task failed.[/bold red] ({time_str})")
